...
Use ARROWS or WASD keys for control.
    ...
    ESC          : quit
    运行时输入法切换为英文
    Y            : record camera1, camera2, data_dic
    E            : all_traffic_lights turn to green
...
try:
    ...
    from pygame.locals import K_e # 魔改，交通灯全变绿
    from pygame.locals import K_f
    from pygame.locals import K_g
    from pygame.locals import K_h
    from pygame.locals import K_i
    from pygame.locals import K_j # 开启一定step后重置世界模式
    from pygame.locals import K_k # 魔改，重置世界
    ...
    from pygame.locals import K_y # 魔改，记录相机
...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        ################################################################镜头队列及超参
        self.step_tick = 0 # 运行帧数计数。放在类中，其他函数调用参数时可以对实例化类的属性直接赋值。不实例化想改类的属性需要用装饰器@classmethod或@staticmethod定义set_XXX函数
        self.im_height = 240 # 前置摄像头图像高
        self.im_width = 320 # 前置摄像头图像宽
        self.record = False # 初始默认不记录影像和字典等
        self._all_green_traffic_lights = False #初始交通灯不设置为全绿
        ...
        #为了销毁传感器方便，按照格式在后续self.sensor中统计
        self.sensor1 = None
        self.sensor2 = None
    ...
    def restart(self):
        self.camera_queue1 = Queue()
        self.camera_queue2 = Queue()
        ...
        if self.player is not None:
            ...
            self.destroy() # 销毁所有的player以及sensor（包含自定义相机1和2）
...
################################################################添加自定义相机
        ...
        self.sensor1 = self.world.spawn_actor(self.rgb_cam, transform, attach_to=self.player)
        self.sensor1.listen(self.camera_queue1.put)
        ...
        self.rgb_cam2.set_attribute("image_size_x", f"{self.im_width}")
        self.rgb_cam2.set_attribute("image_size_y", f"{self.im_height}") 
        self.rgb_cam2.set_attribute("fov", f"110")
        transform2 = carla.Transform(carla.Location(z=20), carla.Rotation(pitch=-90))
        self.sensor2 = self.world.spawn_actor(self.rgb_cam2, transform2, attach_to=self.player)
    ...
    def turn_traffic_lights(self):
        if self._all_green_traffic_lights == False:
            for light_actor in self.light_actor_list:
                light_actor.set_state(carla.TrafficLightState.Green)
                light_actor.freeze(True)
            self._all_green_traffic_lights = True
            print("green_light")
        else:
            for light_actor in self.light_actor_list:
                light_actor.reset_group()
            self._all_green_traffic_lights = False
            print('normal_light')
...
class KeyboardControl(object):
    ...
    def __init__(self, world, start_in_autopilot):
        ...
        self.key_brake_autopilot = False # 默认自驾时未被操作按键打断##############################
    ...
    def keep_traffic_lights(self,world): # 重置后按照保留全绿灯（否则默认重置为正常）
        if world._all_green_traffic_lights: 
            for light_actor in world.light_actor_list:
                light_actor.set_state(carla.TrafficLightState.Green)
                light_actor.freeze(True)
            print("green_light")

    def parse_events(self, client, world, clock, sync_mode, step_tick):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    world.camera_manager.next_sensor()
                elif event.key == K_w and (pygame.key.get_mods() & KMOD_CTRL): # toggle constant velocity mode at 60 km/h. Enabling a constant velocity for a vehicle managed by the Traffic Manager may cause conflicts. This method overrides any changes in velocity by the TM.
                    if world.constant_velocity_enabled: # 已经匀速了就关掉
                        world.player.disable_constant_velocity()
                        world.constant_velocity_enabled = False
                        world.hud.notification("Disabled Constant Velocity Mode")
                    else: # 不是匀速就打开
                        world.player.enable_constant_velocity(carla.Vector3D(17, 0, 0)) # kmh = int(3.6 * math.sqrt(v.x**2 + v.y**2 + v.z**2)) = 61
...
############################################################################################################交通灯全变绿
                    elif event.key == K_e and not pygame.key.get_mods() & KMOD_CTRL:
                        world.turn_traffic_lights()

############################################################################################################世界重置按钮
#重置次数过多会导致崩溃，崩溃不管还能接着操作,但一段时间后服务端会自动关闭。原版手动操控脚本一直backspace也会崩溃。linux可不崩
                    elif event.key == K_k and not pygame.key.get_mods() & KMOD_CTRL:
                        if self._autopilot_enabled:
                            world.player.set_autopilot(False)
                            world.destroy() # 销毁之前的所有player以及sensor（包括自定义相机1和2），self.player = None。
                            world.restart()
                            world.player.set_autopilot(True)
                            print("Autopilot as before")
                        else:
                            world.destroy()
                            world.restart()
...
############################################################################################################世界定时重置
                        ...
                        if not self.step_reset:
                            print("will reset the world")
                            self.step_reset = True
                        
                        else:
                            print("step reset will end")
                            self.step_reset = False

                        # reset_by_step = threading.Thread(target=self.re_world_step,args=(world, step_tick)) 多线程不可行，这里的step_tick是一次性传递，另一线程中不会随着主循环变化.还是得把自动重置放到主循环里
                        # reset_by_step.start()
        ...
        if self.key_brake_autopilot == True: # 如果是被操作键临时打断的自驾，恢复自驾状态
            self.key_brake_autopilot = False

            # new_job = threading.Thread(target=self.re_autopilot(world)) #服务端会掉帧。imshow()会等待延迟，不会漏。同步模式试一下。（同步模式貌似会统一所有线程，调高sleep时间操作卡顿变长；不是同步模式的问题，多线程没有用）这里re_autopilot(world)传参直接调用了该函数，不是并行运行。

            new_job = threading.Thread(target=self.re_autopilot,args=(world,)) #传参学习到了，不等同于直接调用
            new_job.start()
            
    def re_autopilot(self, world): 
        time.sleep(1) 
        self._autopilot_enabled = True
        world.player.set_autopilot(self._autopilot_enabled)
        

    def re_world_step(self, world, step_tick, step_lenth): 
        if step_tick % step_lenth == 0:
            if self._autopilot_enabled:
                world.player.set_autopilot(False)
                world.destroy() # 销毁之前的所有player以及sensor（包括自定义相机1和2），self.player = None。
                world.restart()
    ...
    def _parse_vehicle_keys(self, keys, milliseconds, world): # 参数增加world以便修改自驾模式
        if keys[K_UP] or keys[K_w]:
            # 按了明确的操控键，会暂时终止自驾，并将打断自驾状态改为True
            if self._autopilot_enabled:
                self._autopilot_enabled = False
                world.player.set_autopilot(self._autopilot_enabled)
                self.key_brake_autopilot = True
            else:
                pass
...
class CollisionSensor(object):
    ...
    def get_collision_history(self):
        history = collections.defaultdict(int) #Python中通过Key访问字典，当Key不存在时，会引发‘KeyError’异常。为了避免这种情况的发生，可以使用collections类中的defaultdict()方法来为字典提供默认值。
...
class LaneInvasionSensor(object):
    ...
    def _on_invasion(weak_self, event):
        self = weak_self() # 弱引用不会增加对象的引用数量。引用的目标对象称为 所指对象 （referent）。因此，弱引用不会妨碍所指对象被当作垃圾回收。
        if not self:
            return
        lane_types = set(x.type for x in event.crossed_lane_markings)
        text = ['%r' % str(x).split()[-1] for x in lane_types]
        self.hud.notification('Crossed line %s' % ' and '.join(text))
        self.history.append(event.crossed_lane_markings)
...
def game_loop(args):
    ...
    try:
        ...
        first_tick = True #用于调控第一次循环的部分变量
        world.step_tick = 0
        data_dic = {}
        if not os.path.isdir('./record\\camera1'):
            os.makedirs('./record\\camera1')
        ...
        while True:
            ...
            if controller.parse_events(client, world, clock, args.sync, world.step_tick):
                return
            world.tick(clock)
            ...
            #################################################################碰撞记录、跨道记录清零
            world.lane_invasion_sensor.history.clear()
