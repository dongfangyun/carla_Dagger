...
class CarEnv:
    ...
    def reset(self):
        ...
        self.world.tick() #先tick一下将初始摄像头影像传入sensor_queue_out

        self.episode_start = time.time() #记录每次游戏开始时间
    ...
    def step(self, action, episode_steps): #action.shape([1,2]):  油门刹车action[0][0]:(-1,1) 方向盘action[0][1]:(-1,1)
        ...
        location_step = self.vehicle.get_transform().location
        dist_to_start = location_step.distance(self.location_start)
        ...
        # print(self.done)
        return  self.done, kmh, dist_to_start, self.invation
